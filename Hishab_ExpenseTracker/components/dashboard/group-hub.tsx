...
export function GroupHub() {
  ...
  const handleCreateGroup = async () => {
    ...
    try {
      ...
      const groupId = storageDB.groups.add(groupData);
      
      // Add creator as member
      storageDB.groups.addMember(groupId, {
        userId: user?.uid,
        role: 'admin',
        joinedAt: new Date().toISOString()
      });

      setActiveGroup({ id: groupId, ...groupData });
    } catch (err: any) {
  ...
  const handleJoinGroup = async () => {
    ...
    setError('');
    try {
      const group = storageDB.groups.findByCode(groupCode);
      ...
      // Add as member
      storageDB.groups.addMember(group.id, {
        userId: user?.uid,
        role: 'member',
        joinedAt: new Date().toISOString()
      });
      ...
      <AnimatePresence mode="wait">
        {mode === 'hub' && (
          ...
          >
            ...
            >
              <ArrowLeft className="w-3 h-3 group-hover:-translate-x-1 transition-transform" /> Central Gateway
            </button>

            <div className="space-y-2">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-600 italic">Collective Domain</p>
              <h2 className="text-4xl font-black italic tracking-tighter dark:text-white">Collaborative Tracking</h2>
              <p className="text-xs font-bold text-gray-400 dark:text-slate-600 uppercase tracking-widest leading-relaxed">Choose your entry vector into shared finances.</p>
            </div>
            ...
            <div className="space-y-4">
              ...
              >
                ...
                <div>
                  ...
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mt-1">Be the administrator</p>
                </div>
              </motion.button>
              ...
              >
                ...
                <div>
                  <h3 className="font-black italic text-lg dark:text-white">Join Existing Group</h3>
                  <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 uppercase tracking-widest mt-1">Use 6-digit code</p>
                </div>
              </motion.button>
            </div>
          </motion.div>
        ...
        {mode === 'create' && (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-600 italic">Initialization Phase</p>
              <h2 className="text-3xl font-black italic tracking-tighter dark:text-white">Start a Group</h2>
              <p className="text-xs font-bold text-gray-400 dark:text-slate-600 uppercase tracking-widest">Naming it is the first step</p>
            </div>

            <div className="space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 dark:text-slate-600 uppercase tracking-widest px-4">Group Name</label>
                <input 
                  type="text" 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black text-gray-400 dark:text-slate-600 uppercase tracking-widest px-4">Monthly Budget</label>
                <input 
                  type="number" 
                  placeholder="0.00"
                  value={groupBudget}
              ...
              {error && <p className="text-red-500 text-[10px] font-black uppercase tracking-widest text-center italic">{error}</p>}
            </div>
          </motion.div>
        )}
        ...
        {mode === 'join' && (
          ...
          >
            ...
            <div className="space-y-1">
              <p className="text-[10px] font-black uppercase tracking-[0.4em] text-emerald-600 italic">Authentication Required</p>
              <h2 className="text-3xl font-black italic tracking-tighter dark:text-white">Enter Join Code</h2>
              <p className="text-xs font-bold text-gray-400 dark:text-slate-600 uppercase tracking-widest">Ask your associate for the 6-digit credential</p>
            </div>
            ...
            <div className="space-y-6">
              <input 
                ...
                placeholder="000000"
                value={groupCode}
                onChange={(e) => setGroupCode(e.target.value.replace(/\D/g, ''))}
