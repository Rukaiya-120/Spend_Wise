...
import { Search, Filter, CreditCard, ChevronDown, Download } from 'lucide-react';
  ...
  return (
    <div className="p-4 md:p-8 max-w-4xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-black italic dark:text-white uppercase tracking-tight">Expense history</h1>
      ...
      <div className="space-y-10">
        {groupedExpenses.length > 0 ? (
                  ...
                  >
                    <div className="flex items-center gap-5">
                      <div className={cn(
                        ...
                        expense.category === 'Food' ? "bg-orange-50 dark:bg-orange-950/20 text-orange-500" : 
                        expense.category === 'Bills' ? "bg-blue-50 dark:bg-blue-950/20 text-blue-500" :
                        expense.category === 'Transport' ? "bg-purple-50 dark:bg-purple-950/20 text-purple-500" :
                        "bg-emerald-50 dark:bg-emerald-950/20 text-emerald-500"
                      )}>
                        <CreditCard className="w-6 h-6" />
                      </div>
                      <div>
                        <p className="font-black italic text-sm dark:text-white uppercase tracking-tight">{expense.note || expense.category}</p>
                        <p className="text-[10px] font-bold text-gray-400 dark:text-slate-500 flex items-center gap-2 mt-0.5">
                          <span className="uppercase tracking-widest text-emerald-600 font-black italic">{expense.category}</span>
                    ...
                    <div className="text-right">
                      <p className="font-black italic text-sm dark:text-white uppercase tracking-tight">-{expense.amount.toLocaleString()} {currency}</p>
                      <div className="flex items-center justify-end gap-1.5 mt-1 border border-gray-100 dark:border-slate-800 rounded-full px-2 py-0.5 inline-flex">
                        <div className={cn("w-1.5 h-1.5 rounded-full", expense.type === 'personal' ? "bg-blue-500" : "bg-emerald-500")} />
          ...
          <div className="text-center py-20 bg-white dark:bg-slate-900 rounded-[3rem] border-2 border-dashed border-gray-100 dark:border-slate-800 space-y-6">
            ...
            <p className="text-[10px] font-black text-gray-400 dark:text-slate-500 uppercase tracking-[0.4em]">Historical records not found</p>
          </div>
        )}
      </div>
    </div>
